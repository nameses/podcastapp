...
class GetNewPodcastListUseCase @Inject constructor(private val podcastRepository: PodcastRepository) {
    operator fun invoke(page: Int) = flow<UiEvent<PodcastList>> {
        emit(UiEvent.Loading())

        when (val response = podcastRepository.getPodcastList(PodcastType.New, page)) {
